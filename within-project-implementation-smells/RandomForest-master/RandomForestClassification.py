...
if __name__ == '__main__':
    ...
    def getTrainOrTestCSV(datalist, csv_txt, jsonFolderPath):
        label_dict = {}
        ...
        for root ,dirs, files in os.walk(jsonFolderPath):
            for file in files:
                ...
                if file in label_dict:
                    label = 1 if label_dict[file] == '1\n' else 2
                    csv_data.append(','.join(map(lambda x:str(x), src_metrics))+','+str(label)+'\n')
        data_txt = open(csv_txt,'w')
        data_txt.writelines(csv_data)
        data_txt.close()

    record_file = open('result.txt', 'a')
    for project in projectList:
        
        train_list_path = "/home/yqx/Downloads/DesigniteJava-master/myData/"+project+"/methods/trainlabels.txt"
        test_list_path = "/home/yqx/Downloads/DesigniteJava-master/myData/"+project+"/methods/testlabels.txt"
        jsonFolderPath = "/home/yqx/Downloads/DesigniteJava-master/myData/"+project+"/methods/rawjson"
        '''
        saveModelsPath = "/home/yqx/Desktop/TD-data-process/outDataMethod/"+project+"/trainedModels"
        ...
        getTrainOrTestCSV(trainlist, 'train_csv.txt', jsonFolderPath)
        getTrainOrTestCSV(testlist, 'test_csv.txt', jsonFolderPath)
        ...
        #print("test_confusion_matrix", metrics.confusion_matrix(test_data.loc[:, 'label'], clf.predict(test_data.loc[:, feature_list])))
        ...
        #record_file.write("test_confusion_matrix"+ str(metrics.confusion_matrix(test_data.loc[:, 'label'], clf.predict(test_data.loc[:, feature_list])))+'\n')
