...
class GlobalSelfAttentionLayer(nn.Module):
    ...
    def forward(self, all_token_tensor): # features是一个二维列表
        ...
        #print('W_all_token_tensor',W_all_token_tensor.shape)
        ...
        #print(len(W_all_token_tensor),attention.reshape(len(W_all_token_tensor),-1))
        #global_vec_tensor = F.elu(torch.matmul(attention, W_all_token_tensor).view(1,-1)) #（1*N · N*f = 1*f）
        #print('global_vec_tensor',global_vec_tensor.shape)
        #return global_vec_tensor  # 返回所有结点的单一向量组成的二维张量特征数据

        global_vec_tensor = attention.reshape(len(W_all_token_tensor),-1)*W_all_token_tensor
        return global_vec_tensor
