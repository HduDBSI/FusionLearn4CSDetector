...
class MyGCN(nn.Module):
    ...
    def forward(self, x, edge_index, edge_attr):
        ...
        output = torch.stack(readout,dim=0).squeeze(1)
        #print('readout',output.shape,output)
        return output
...
class GCN_Transformer_fusion(nn.Module):
    ...
    def forward(self, x, edge_index, edge_attr, metrics):
        ...
        #out1 = self.attention1(out1).reshape(1,-1)
        ...
        #fusion = torch.stack((out1,out2),dim=0).squeeze(1)
        #print('fusion',fusion.shape)
        #att = self.attention(fusion).reshape(1,-1)
        
        #output =  self.fc(att)

        fusion = torch.cat((out1,out2),dim=1)
        output =  self.fc(fusion)
        #output =  torch.mm(fusion, self.Wo)
        
        result = F.softmax(output,dim=-1)
        
        return result

class GCN_Transformer_fusion_Att(nn.Module):
    def __init__(self, vocablen, metrilen, hidden, d_model, nhead, num_encoder_layers, dim_feedforward, dropout, alpha):
        super(GCN_Transformer_fusion_Att, self).__init__()
        self.device = 'cuda'
        self.gcn = MyGCN(hidden)
        self.transformer = MyTransformerEncoder(d_model, nhead, num_encoder_layers, dim_feedforward, dropout)
...
class GCN_Transformer_fusion_edge(nn.Module):
    def __init__(self, vocablen, metrilen, hidden, d_model, nhead, num_encoder_layers, dim_feedforward, dropout, alpha):
        super(GCN_Transformer_fusion_edge, self).__init__()
        self.device = 'cuda'
        self.gcn = MyGCN_edge(hidden)
        self.transformer = MyTransformerEncoder(d_model, nhead, num_encoder_layers, dim_feedforward, dropout)
...
class TransformerOnly(nn.Module):
    def __init__(self, vocablen, metrilen, hidden, d_model, nhead, num_encoder_layers, dim_feedforward, dropout, alpha):
        ...
        self.dense2 = nn.Linear(11*hidden, hidden)
...
class BiLSTM_Self_Attention(nn.Module):
    def __init__(self, vocablen, metrilen, hidden, dropout, alpha):
        ...
        self.attention = GlobalSelfAttentionLayer(2*hidden, 2*hidden, dropout, alpha)
...
class BiLSTM_Self_Attention_token(nn.Module):
    ...
    def forward(self, token_list):
        ...
        return out
class GCN_Transformer_fusion_v3(nn.Module):
    def __init__(self, vocablen, metrilen, hidden, d_model, nhead, num_encoder_layers, dim_feedforward, dropout, alpha):
        super(GCN_Transformer_fusion_v3, self).__init__()
        self.device = 'cuda'
        self.gcn = MyGCN(hidden)
        self.transformer = MyTransformerEncoder(d_model, nhead, num_encoder_layers, dim_feedforward, dropout)

        self.transformer_out = MyTransformerEncoder(d_model, nhead, num_encoder_layers, dim_feedforward, dropout)
    ...
    def forward(self, x, edge_index, edge_attr, metrics):
        ...
        fusion = torch.cat((out1,out2), dim=1).reshape(14,-1).unsqueeze(1)
        #fusion = torch.stack((out1,out2),dim=0).squeeze(1)
        #print('fusion',fusion.shape)
        #exit()
        #simple_attention_out = F.softmax(torch.matmul(fusion, self.ao),dim=0)
        #fusion = torch.mm(simple_attention_out.T, fusion)
        #batch = torch.zeros(fusion.size(0),dtype=torch.long).to(self.device)
        #fusion = self.bi_lstm(fusion).squeeze(1)
        fusion = self.transformer_out(fusion).squeeze(1)
        batch = torch.zeros(fusion.size(0),dtype=torch.long).to(self.device)
        out = self.gpool(fusion, batch)
        out = self.attention(out)
        #out = fusion.reshape(1,-1)
        #out = self.attention(out)
...
class GCN_FAAST(nn.Module):
    def __init__(self, vocablen, metrilen, hidden, d_model, nhead, num_encoder_layers, dim_feedforward, dropout, alpha):
        super(GCN_FAAST, self).__init__()
        self.device = 'cuda'
        self.gcn = MyGCN(hidden)
        #self.transformer = MyTransformerEncoder(d_model, nhead, num_encoder_layers, dim_feedforward, dropout)
    ...
    def forward(self, x, edge_index, edge_attr, metrics):
        ...
        return result
class Transformer_metrics(nn.Module):
    def __init__(self, vocablen, metrilen, hidden, d_model, nhead, num_encoder_layers, dim_feedforward, dropout, alpha):
        super(Transformer_metrics, self).__init__()
        self.device = 'cuda'
        #self.gcn = MyGCN(hidden)
        self.transformer = MyTransformerEncoder(d_model, nhead, num_encoder_layers, dim_feedforward, dropout)
...
class MyModel(nn.Module):
    def __init__(self, vocablen, metrilen, hidden, d_model, nhead, num_encoder_layers, dim_feedforward, dropout, alpha):
        ...
        #self.gcn_transformer_fusion_edge = GCN_Transformer_fusion_edge(vocablen, metrilen, hidden, d_model, nhead, num_encoder_layers, dim_feedforward, dropout, alpha)
        #self.gcn_transformer_fusion_v2 = GCN_Transformer_fusion_Att(vocablen, metrilen, hidden, d_model, nhead, num_encoder_layers, dim_feedforward, dropout, alpha)
        ...
        #self.transformer = TransformerOnly(vocablen, metrilen, hidden, d_model, nhead, num_encoder_layers, dim_feedforward, dropout, alpha)
        #self.gcn_only = GCN_FAAST(vocablen, metrilen, hidden, d_model, nhead, num_encoder_layers, dim_feedforward, dropout, alpha)
        #self.transformer_only = Transformer_metrics(vocablen, metrilen, hidden, d_model, nhead, num_encoder_layers, dim_feedforward, dropout, alpha)
    ...
    def forward(self, x, edge_index, edge_attr, metrics, token_list, src_metrics):
        ...
        #output = self.gcn_transformer_fusion_edge(x, edge_index, edge_attr, metrics)
        #output = self.gcn_transformer_fusion_v2(x, edge_index, edge_attr, metrics)
        ...
        #output = self.transformer(x, edge_index, edge_attr, metrics)
        #output = self.gcn_only(x, edge_index, edge_attr, metrics)

        #output = self.transformer_only(x, edge_index, edge_attr, metrics)
