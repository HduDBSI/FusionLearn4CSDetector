...
#projectList = ['Ant','flink','gradle','hadoop','hbase','jruby','kafka','mockito','storm','tomcat'][9:]
...
def Undersampling(training_set):
    ...
    if pos>=neg:
        ...
        print('after sampling(pos:neg): ',len(posSamples[:neg]),':',len(negSamples))
    elif neg>pos:
        selectSamples = negSamples[:pos] + posSamples
        print('after sampling(pos:neg): ',len(posSamples),':',len(negSamples[:pos]))
