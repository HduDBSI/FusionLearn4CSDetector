...
def preprocess_data(train_labels, test_labels, metohdfilepathList):
    ...
    ast_list = []
    label_list = []
    for methodfilepath in metohdfilepathList:
        for root, ds, files in os.walk(methodfilepath):
            
            for file in tqdm(files):
                if file.endswith('.java') and file in label_dict:
                    #print('file.endswith() and file in label_dict')
                    codepath = root + '/' + file
                    #print(codepath)
                    
                    if os.path.getsize(codepath) > 1024*10:
                        print("文件大于10kb")
                        continue
