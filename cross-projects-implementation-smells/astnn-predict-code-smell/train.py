...
def init_data_and_model(projectname):
    ...
    #train_labels = "/home/yqx/Desktop/TD-data-process/outDataMethod/"+projectname+"/"+"trainlabels.txt"
    #test_labels = "/home/yqx/Desktop/TD-data-process/outDataMethod/"+projectname+"/"+"testlabels.txt"
    
    
    metohdfilepathList = []
    for projectname in projectList:
        metohdfilepathList.append('/home/yqx/Downloads/DesigniteJava-master/myData/'+ projectname +'/methods/pureJavaMethod')

    w2v, programs = preprocess_data(train_labels, test_labels, metohdfilepathList)
...
def Undersampling(training_set):
    ...
    if pos>=neg:
        slcienum = int(neg*sample_rate)
        selectSamples = negSamples[:slcienum] + posSamples[:slcienum]
        print('after sampling(pos:neg): ',len(posSamples[:slcienum]),':',len(negSamples[:slcienum]))
    elif neg>pos:
        slcienum = int(pos*sample_rate)
        selectSamples = negSamples[:slcienum] + posSamples[:slcienum]
        print('after sampling(pos:neg): ',len(posSamples[:slcienum]),':',len(negSamples[:slcienum]))
...
if __name__ == '__main__':
    ...
    for projectname in projectList:
        ...
        test_result = '/home/yqx/Downloads/DesigniteJava-master/myData/'+ projectname +'/_method_10_cross_.txt'
        for i in range(1):
