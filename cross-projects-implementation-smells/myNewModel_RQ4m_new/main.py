...
def get_10_fold_cross_train_test_list(projectList):
    ...
    for projectname in projectList:
        train_list_path = "/home/yqx/Downloads/comment_data/comment-data/DesigniteJava-master/myData/"+projectname+"/methods/trainlabels.txt"
        trainlist += open(train_list_path).readlines()
        train_list_path = "/home/yqx/Downloads/comment_data/comment-data/DesigniteJava-master/myData/"+projectname+"/methods/testlabels.txt"
        trainlist += open(train_list_path).readlines()
...
def init_data_and_model(projectList):
    jsonFolderPathList = []
    for projectname in projectList:
        jsonFolderPathList.append('/home/yqx/Downloads/comment_data/comment-data/DesigniteJava-master/myData/'+ projectname +'/methods/rawjson')
...
def init_testdata_and_model(projectList_train, project_test):
    ...
    project_test = '/home/yqx/Downloads/comment_data/comment-data/DesigniteJava-master/myData/'+ project_test +'_new/methods/rawjson'
...
def test(model, testlist, model_index, allJsonDict, batch_size):
    model.load_state_dict(torch.load('./models_new/ gcn_trainsformer_fusion epoch'+str(model_index)+'.pkl'))
    with torch.no_grad():
...
def train(trainlist):
    ...
    for epoch in epochs:
        ...
        for batch_index in tqdm(range(int(len(train_data)/args.batch_size))):
            ...
            iterations += 1
        if epoch>25:
            saveModelsPath = '/home/yqx/Downloads/comment_data/comment-data/DesigniteJava-master/myData/A_myModel_method_10_cross/myNewModel_RQ4m/models_new'
...
if __name__ == '__main__':
    ...
    if train_flag:
        ...
        model_index = 0
        modelNames = [' gcn_trainsformer_fusion ',' Dense(CNN) ',' BiLSTM+attention ']
        modelName = modelNames[model_index]
        
        trainlist, allJsonDict, model = init_data_and_model(projectList)
        #model.load_state_dict(torch.load('./models_new/ gcn_trainsformer_fusion epoch'+str(29)+'.pkl'))
        train(trainlist)
    else:
        #test
...
'Long Identiﬁer','Long Method','Long Parameter List','Long Statement','Magic Number','Missing default']
        ...
        for project in projectList:
            ...
            for current_smell in methodSmellList:
                ...
                test(model, new_testlist, model_index, allJsonDict, 1)

            print("project_overall_performance:\n", project)
            test(model, project_overall_testlist, model_index, allJsonDict, 1)