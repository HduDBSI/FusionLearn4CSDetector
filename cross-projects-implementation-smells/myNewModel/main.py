...
def init_data_and_model(project):
    ...
    test_result = '/home/yqx/Downloads/DesigniteJava-master/myData/'+ project +'/_method_10_cross_.txt'
    ...
    return test_result, trainlist, testlist, allJsonDict, model, saveModelsPath
...
def train():
    ...
    for epoch in epochs:
        ...
        if (int(epoch)==29):
            ...
            test_p_r_f1.write(modelName+str(29) +" "+ str(p) +" "+ str(r) +" "+ str(f1)+"\n")
            test_p_r_f1.close()
    p, r, f1 = test(testlist, epoch, allJsonDict, 1)
    test_p_r_f1 = open(test_result, 'a')
...
if __name__ == '__main__':
    ...
    num = 2.0
    for project in projectList[5:]:
        p_sum = r_sum = f1_sum = 0
        for i in range(int(num)):
            print("projectName:",project)
            test_result, trainlist, testlist, allJsonDict, model, saveModelsPath = init_data_and_model(project)
            testlist = testlist[:len(testlist)//2]
            p,r,f1 = train()
            p_sum += p
            r_sum += r
            f1_sum += f1
